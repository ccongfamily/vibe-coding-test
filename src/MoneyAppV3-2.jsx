          ...
          <>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
              ...
              <button onClick={()=>setSelected(
                ...
                {selected.length===parsed.length?"전체해제":"전체선택"}
              </button>
            </div>
        ...
        {step === "done" && (
          <div style={{textAlign:"center",padding:"80px 0"}}>
            ...
            <p style={{color:"#10B981",fontSize:"18px",fontWeight:"700",margin:"0 0 6px"}}>
              {selected.length}건 추가됐어요!
            </p>
            <p style={{color:"#94A3B8",fontSize:"13px"}}>가계부에 반영됐어요</p>
          </div>
        )}
      </div>
      ...
      {step === "preview" && (
        <div style={{
          ...
          <button onClick={handleConfirm} disabled={selected.length===0} style={{
            ...
            {selected.length>0
              ?`${selected.length}건 가계부에 추가 (${Math.round(totalAmt/10000)}만원)`
              :"항목을 선택해주세요"}
          </button>
        </div>
      )}
...
export default function MoneyAppV2() {
  ...
  const showNav = screen !== "txlist"; // 전체내역 화면은 자체 뒤로가기 사용
